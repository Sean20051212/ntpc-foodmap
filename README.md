...
| DB-3 ✅ | `districts` 移除經緯度（2026-06-07 完成）| 拿掉 `center_latitude/center_longitude` 兩欄；前端 focusDistrict 需要的中心座標改由 `/api/dicts/districts` 端點動態 `AVG(restaurants.lat/lng)` 算出，response shape 不變 |
...
| DB-8 ✅ | ERD 正規化審查（2026-06-07 完成）| 詳見 [docs/erd-normalization.md](docs/erd-normalization.md)：13 張表 1NF/2NF/3NF/BCNF/4NF/5NF 全達成；rating_avg/rating_count 與 photos.local_path 為刻意反正規化（trigger 維護 / fallback 設計）；順手補上 districts/days_of_week 缺漏的 UNIQUE |
...
| BE-4 ✅ | 距離搜尋優化（2026-06-07 完成）| `lib/restaurants.php` 新增 `restaurantCandidateZipcodes()`；當 `max_distance_m` 設定且使用者沒手動勾 district 時，自動以 `user_zipcode + 鄰接區` 當候選池前置過濾，避免對全 687 家跑 Haversine；前端 `params()` 帶 `user_zipcode: locInfo?.district?.zipcode` |
...
| # | 項目 | 說明 |
|---|---|---|
| FE-1 ✅ | 抽獎輪盤（2026-06-07 完成）| (1) 無圖時顯示預設圖；(2) 輪盤上顯示各候選餐廳名稱 |
| FE-2 ✅ | 登入頁清理（2026-06-07 完成）| 移除「測試帳號」「管理員帳號」說明文字 |
| FE-3 ✅ | 移除技術性說明文字（2026-06-07 完成）| 各頁面對一般使用者無意義的技術描述已刪 |
| FE-4 ✅ | 地圖頁桌面版排版（2026-06-07 完成）| 修正「搜尋此區域」按鈕跑版問題 |
| FE-5 ✅ | 管理員操作確認（2026-06-07 完成）| 刪除 / 修改前加入 `confirmDialog`（PR #9 實作）|
