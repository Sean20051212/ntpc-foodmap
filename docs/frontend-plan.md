...
## ⚠️ 2026-06-02 待辦（推翻 / 補充部分舊規格）

對應 README「待辦事項清單」FE-1 ~ FE-5、BE-1、BE-3。下列項目尚未實作。

| # | 項目 | 影響章節 |
|---|---|---|
| FE-1 | 抽獎輪盤修正：(1) 被抽中的餐廳若無圖片，顯示預設圖（不是空白）；(2) 輪盤動畫上要顯示各候選餐廳的名稱 | §3.5 map.php 輪盤 |
| FE-2 | 登入頁移除「測試帳號」「管理員帳號」等說明文字 | §3.2 login.php |
| FE-3 | 各頁面刪除過於技術性的說明文字（例：「密碼以雜湊演算法加密」） | 全頁面 |
| FE-4 | 地圖頁桌面版：點擊「搜尋此區域」按鈕時的跑版問題需修正 | §3.5 map.php |
| FE-5 | 管理員執行刪除 / 修改前必須加 confirm dialog（目前 admin §3.8 有提到「操作前都 confirm dialog」，但實際未實作完整） | §3.8 admin.php |
| BE-1（前端側） | 重新輸入 keyword / 地址，或按下搜尋鈕時，**前端要主動清空所有篩選 state**（距離、評分、區域、分類），不再要求使用者按「清除篩選」 | §3.4 search.php |
| BE-3（前端側） | 拿掉任何寫死板橋座標的 fallback；改為一律先 `navigator.geolocation.getCurrentPosition()` 取得 GPS，使用者拒絕授權再顯示「請輸入地址」的 UI | §3.4 / §3.5 開頁邏輯 |
