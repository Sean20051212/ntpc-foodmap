...
function ProfilePage() {
  ...
  useEffect(() => {
    requireCurrentUser()
      ...
      .catch(error => setErr(readableError(error, "讀取使用者資料失敗")));
  }, []);
  ...
  return (
    <>
      <Nav active="" loggedIn={true} userName={displayName}/>
      <main className="page">
        ...
        {!user && !err && (
          <div className="empty">
            <div className="empty-icon"><Icon name="user" size={42} stroke={1.5}/></div>
            <h3 className="empty-text">讀取中…</h3>
          </div>
        )}

        {err && (
          <div className="empty">
            <div className="empty-icon"><Icon name="error" size={42} stroke={1.5}/></div>
            <h3 className="empty-text">{err}</h3>
            <a href="login.php" className="btn btn-primary">重新登入</a>
          </div>
        )}
